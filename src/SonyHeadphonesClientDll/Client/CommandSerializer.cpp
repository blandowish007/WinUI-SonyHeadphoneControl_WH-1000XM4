...
namespace CommandSerializer
{
	...
	Buffer extractPayload(const Buffer& unescapedFrame)
	{
		// Frame layout (unescaped): dataType(1) + seq(1) + sizeBE4(4) + payload(size) + checksum(1)
		if (unescapedFrame.size() < 7)
		{
			throw std::runtime_error("Invalid frame: smaller than minimum frame size");
		}
		unsigned int payloadSize = static_cast<unsigned char>(unescapedFrame[2]) << 24;
		payloadSize |= static_cast<unsigned char>(unescapedFrame[3]) << 16;
		payloadSize |= static_cast<unsigned char>(unescapedFrame[4]) << 8;
		payloadSize |= static_cast<unsigned char>(unescapedFrame[5]);
		size_t payloadStart = 6;
		size_t payloadEnd = payloadStart + payloadSize;
		if (payloadEnd >= unescapedFrame.size())
		{
			throw std::runtime_error("Invalid frame: payload size exceeds frame length");
		}
		return Buffer(unescapedFrame.begin() + payloadStart, unescapedFrame.begin() + payloadEnd);
	}
	...
	Buffer serializeBatteryLevelQuery()
	{
		// [COMMON_GET_BATTERY_LEVEL, BATTERY_INQUIRED_TYPE::BATTERY] = [0x10, 0x00]
		Buffer ret;
		ret.push_back(static_cast<unsigned char>(COMMAND_TYPE::COMMON_GET_BATTERY_LEVEL));
		ret.push_back(static_cast<unsigned char>(BATTERY_INQUIRED_TYPE::BATTERY));
		return ret;
	}
