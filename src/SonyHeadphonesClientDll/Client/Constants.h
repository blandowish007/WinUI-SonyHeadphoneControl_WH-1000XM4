...
enum class COMMAND_TYPE : signed char
{
	VPT_SET_PARAM = 72,
	NCASM_SET_PARAM = 104,
	// Battery query: payload [COMMON_GET_BATTERY_LEVEL, BATTERY_INQUIRED_TYPE] = [0x10, 0x00]
	COMMON_GET_BATTERY_LEVEL = 16,
	// Battery response (RET): payload [0x11, inquired_type, battery_level, charge_status]
	COMMON_RET_BATTERY_LEVEL = 17,
	// Battery unsolicited notification (NTFY): same payload layout as RET
	COMMON_NTFY_BATTERY_LEVEL = 19
};

// Battery inquiry type: BATTERY = single-cell device battery level (XM4)
