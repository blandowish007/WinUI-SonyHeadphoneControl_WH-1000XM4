...
bool Headphones::setChanges()
{
	bool refresh_success = this->_conn.refresh();
	if (!refresh_success)
		return false;
...
int Headphones::queryBatteryLevel()
{
	// Send battery query and read the RET frame; battery percentage is at payload byte[2].
	auto frame = this->_conn.sendCommandWithResponse(CommandSerializer::serializeBatteryLevelQuery());
	if (frame.empty())
	{
		return -1;
	}
	auto payload = CommandSerializer::extractPayload(frame);
	// Expected payload = [0x11, inquired_type, battery_level, charge_status]
	if (payload.size() < 3)
	{
		return -1;
	}
	int level = static_cast<unsigned char>(payload[2]);
